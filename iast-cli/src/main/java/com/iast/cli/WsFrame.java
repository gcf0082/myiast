...
/**
 * 最小 RFC 6455 text/close frame 编解码（客户端副本）。
 *
 * <p>和 {@code com.iast.agent.cli.WsFrame}（服务端版本，驻 iast-agent.jar 里）**逐字节同义**。
 * 之所以两边各持一份而不是做公共 jar：
 * <ul>
 *   <li>服务端只用 server-side 编码路径（不 mask），客户端只用 client-side（mask）</li>
 *   <li>共享 jar 会给部署多出一个文件 / 引入运行期耦合</li>
 *   <li>RFC 6455 text/close/ping/pong 子集协议稳定，150 行代码漂移风险低</li>
 * </ul>
 * 若两边协议有分歧，PR review 时需同时比对 agent 和 cli 两侧 WsFrame.java。
 *
